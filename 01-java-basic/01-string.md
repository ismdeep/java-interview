## 第 01 讲：String 的特点是什么？它有哪些重要的方法？
...
3. `compareTo()` 比较两个字符串
    ...
    从源码中可以看出，`compareTo()` 方法会循环对比所有的字符，当两个字符串中有任意一个字符不相同时，则 `return char1 - char2`. 比如，两个字符串分别存储的是 1 和 2，返回的值是 -1；如果存储的是 1 和 1 ，则返回的值是 0 ，如果存储的是 2 和 1 ，则返回值是 1.
